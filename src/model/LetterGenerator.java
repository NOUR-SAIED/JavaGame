...
public class LetterGenerator {
    ...
    public static List<String> loadWordsByLength(int length) {
        List<String> words = new ArrayList<>();

        // Access the dictionary file as a resource
        String resourcePath = "/" + length + "_LETTRES.txt";

        try (BufferedReader br = new BufferedReader(new InputStreamReader(
                LetterGenerator.class.getResourceAsStream(resourcePath)))) {
            String line;
            while ((line = br.readLine()) != null) {
                words.add(line.trim().toUpperCase());
            }
        } catch (IOException | NullPointerException e) {
            System.err.println("Erreur lors du chargement du fichier ressource : " + resourcePath + ". " + e.getMessage());
        }
